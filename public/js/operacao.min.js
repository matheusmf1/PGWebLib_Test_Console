...
input.className='form__ProjContent--input';divTitle.appendChild(label);divTitle.appendChild(input);formNode.appendChild(divTitle);const divDescription=document.createElement('div');divDescription.className='form__ProjContent';const selectProj=document.createElement('select');selectProj.name="projectTitle";selectProj.id="selectProj";await fetch('/main/projs',{method:'GET'}).then((response)=>{return response.json();}).then((resp)=>{console.log('projects',resp);resp.projects.forEach((proj)=>{let option=document.createElement('option');option.value=proj.title;option.innerHTML=proj.title;selectProj.appendChild(option);});divDescription.appendChild(selectProj);formNode.appendChild(divDescription);}).catch((err)=>{console.log('Erro Fetch',err);});const jsonData=document.createElement('input');jsonData.name="info";jsonData.style="display: none";jsonData.value=valData;formNode.appendChild(jsonData);const button=document.createElement('button');button.className='button-login button-block';button.setAttribute('type','submit');button.setAttribute('id','submitBtn');button.innerHTML='Adicionar';const anchor=document.createElement('a');anchor.href='';anchor.className='modal__close demo-close modal__anchor';anchor.innerHTML=`<svg viewBox="0 0 24 24">
