...
(()=>{const reloadAuth=()=>{setInterval(()=>{location.reload();},3600000);}});

(()=>{const update=setInterval(async()=>{await fetch('/operacao/resultado',{method:'GET',headers:{'Content-Type':'application/json'},mode:'cors'}).then((response)=>{if(response.status===200)
...
(()=>{const status=setInterval(async()=>{await fetch('/operacao/status',{method:'GET',headers:{'Content-Type':'application/json'}}).then((response)=>{return response.json();}).then((resp)=>{console.log('Status',resp);if(resp.erro){let sectionNode=document.getElementById('comprovante__container');let ulNode=document.createElement('ul');resp.erro.forEach((c)=>{let liNode=document.createElement('li');liNode.classList='comprovante__lista--titulo';liNode.innerHTML=Object.values(c).toString();ulNode.appendChild(liNode);});sectionNode.appendChild(ulNode);clearInterval(status);}}).catch((err)=>{console.log('Erro Fetch',err);});},10000);})();

(()=>{const validation=setInterval(async()=>{await fetch('/operacao/validacao',{method:'GET',headers:{'Content-Type':'application/json'}}).then((response)=>{return response.json();}).then((resp)=>{console.log('validacao',resp)
